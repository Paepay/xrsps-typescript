...
/**
 * Minimal Romeo interaction to prove dialog + options flow.
 */
export const romeoModule: ScriptModule = {
    id: "content.romeo",
    register(registry, services) {
        // Prevent overlapping conversations per player
        ...
        const playRomeoConversation = (event: any) => {
            ...
            openNpcDialog(
                ...
                () => {
                    // Close the intro dialog so the options are clearly visible
                    services.closeDialog?.(event.player, `${convoId}_intro`);

                    // Step 2: Player choices
        ...
        // Fallback when option string is missing/normalized differently
        registry.registerNpcScript({
            npcId: 5037,
            option: undefined,
            handler: playRomeoConversation,
