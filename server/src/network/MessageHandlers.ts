    ...
    return (ctx) => {
        try {
            ...
            if (text.startsWith("::")) {
                ...
                if (root === "anim") {
                    const animIdArg = parts[1];
                    const animId = animIdArg ? parseInt(animIdArg, 10) : NaN;

                    if (!Number.isFinite(animId)) {
                        services.queueChatMessage({
                            messageType: "game",
                            text: "Usage: ::anim <sequenceId> — e.g. ::anim 855 (Yes emote)",
                            targetPlayerIds: [sender.id],
                        });
                        return;
                    }

                    sender.queueOneShotSeq(animId, 0);
                    services.queueChatMessage({
                        messageType: "game",
                        text: `Playing animation ${animId}`,
                        targetPlayerIds: [sender.id],
                    });
                    logger.info(`[cmd] ::anim - Player ${sender.id} played animation ${animId}`);
                    return;
                }
