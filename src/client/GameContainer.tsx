...
import {
    ...
    subscribeRegionalFavourHud,
    type RegionalFavourHudEvent,
...
export function GameContainer({ osrsClient }: OsrsContainerProps): JSX.Element {
    ...
    const [regionalFavourHud, setRegionalFavourHud] = useState<RegionalFavourHudEvent | null>(null);
    const [regionalFavourHudMinimized, setRegionalFavourHudMinimized] = useState(false);
    const [regionalFavourShowRewards, setRegionalFavourShowRewards] = useState(false);
    ...
    useEffect(() => {
        return subscribeRegionalFavourHud((event) => {
            if (!event?.visible) {
                setRegionalFavourHud(null);
                setRegionalFavourShowRewards(false);
                return;
            }
            setRegionalFavourHud(event);
            if (!event.hasActiveFavour) {
                setRegionalFavourShowRewards(false);
            }
        });
    }, []);
            ...
            <div className={isMobileMode ? "game-viewport game-viewport-mobile" : "game-viewport"}>
                <div className="game-canvas-shell">
                    <div className="game-canvas-stage">
                        ...
                        {!hideUi && (
                            <div className="hud left-bottom regional-favour-hud-anchor">
                                {!regionalFavourHud || regionalFavourHudMinimized ? (
                                    <button
                                        type="button"
                                        className="regional-favour-hud regional-favour-hud-minimized content-text"
                                        title="Show favour HUD"
                                        onClick={() => {
                                            setRegionalFavourHudMinimized(false);
                                            if (!regionalFavourHud) {
                                                sendChat("::favour show");
                                            }
                                        }}
                                    >
                                        +
                                    ...
                                    <div className="regional-favour-hud content-text">
                                        <div className="regional-favour-hud-header">
                                            <div className="label">
                                            ...
                                            <div className="regional-favour-hud-actions">
                                                <button
                                                    ...
                                                    title="Minimize"
                                                    onClick={() => {
                                                        setRegionalFavourHudMinimized(true);
                                                        setRegionalFavourShowRewards(false);
                                                    }}
                                                >
                                                    -
                                                ...
                                                <button
                                                    type="button"
                                                    className={`regional-favour-hud-btn${
                                                        regionalFavourShowRewards ? " active" : ""
                                                    }`}
                                                    title={
                                                        regionalFavourHud.hasActiveFavour
                                                            ? "Show rewards"
                                                            : "No active task"
                                                    }
                                                    disabled={!regionalFavourHud.hasActiveFavour}
                                                    onClick={() =>
                                                        setRegionalFavourShowRewards((v) => !v)
                                                    }
                                                >
                                                    $
                                                </button>
                                                <button
                                                    type="button"
                                                    className="regional-favour-hud-btn regional-favour-hud-btn-cancel"
                                                    title={
                                                        regionalFavourHud.hasActiveFavour
                                                            ? "Cancel favour"
                                                            : "No active task"
                                        ...
                                        <div className="objective">{regionalFavourHud.objective}</div>
                                        {regionalFavourHud.required > 0 && (
                                            <div className="progress">
                                                Progress: {regionalFavourHud.progress}/
                                                {regionalFavourHud.required}
                                                {regionalFavourHud.complete ? " (complete)" : ""}
                                            </div>
                                        )}
                                        {regionalFavourHud.complete &&
                                            regionalFavourHud.turnInName && (
                                                <div className="turn-in">
                                                    Return to {regionalFavourHud.turnInName}
                                                </div>
                                            )}
                                        {!regionalFavourHud.complete &&
                                            regionalFavourHud.required === 1 &&
                                            regionalFavourHud.progress === 0 &&
                                            regionalFavourHud.turnInName && (
                                                <div className="turn-in">
                                                    Regional contact: {regionalFavourHud.turnInName}
                                                </div>
                                            )}
                                        {regionalFavourShowRewards &&
                                            regionalFavourHud.rewardPreview && (
                                                <div className="reward">
                                                    Reward: {regionalFavourHud.rewardPreview}
                                                </div>
                                            )}
                                        {regionalFavourHud.instruction && (
